        ...
        <div class="picker-panel" v-show="state===1" @click.stop>
          ...
          <div class="picker-content">
            ...
            <div class="wheel-wrapper" ref="wheelWrapper">
              <div class="wheel" v-for="(data,i) in pickerData" v-bind:key="i">
                <ul class="wheel-scroll">
                  <li v-for="(item,j) in data" class="wheel-item" v-bind:key="j">{{item.text}}</li>
                </ul>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    setSelectedIndex (index) {
      ...
      console.log(this.pickerSelectedIndex)
    },
    refill (datas) {
      let ret = []
      if (!datas.length) {
